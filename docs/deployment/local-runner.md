...
首页将原“分析报告”和“最近发布”合并为一张“分析记录”表，每个发布任务只占一行。表中分别展示报告名称、来源、分析结果、保存状态和时间，并提供查看和私有下载。每日 Actions 来源显示“每日定时”，主 Runner 显示“网页分析”；旧记录缺少来源或执行摘要时显示“未记录”，不推断为全部完成。

“已保存”只表示报告保存成功；综合分析的“全部完成／部分完成”来自执行摘要。保存中、保存失败且尚无报告的任务仍展示，但不提供无效的查看或下载入口。分页按保存任务更新时间排序，报告存在时显示生成时间，否则显示状态更新时间。列表只读取索引和摘要，正文及附件按需鉴权加载；数据库和 Storage 的用户隔离保持不变。

本次仅修改前端，无新增环境变量、数据库迁移或历史数据变更。回滚到上一版 Vercel 部署即可恢复原两块展示，不需要回滚数据库。此中文专题没有对应英文版本，未修改 README。
