...
顶栏「DSA 研究档案」右侧提供紧凑的刷新按钮，刷新整个页面并重新读取数据。首页移除「分析工作台」标题与介绍文案，左侧自选股和右侧发起分析从顶栏下方直接开始，右侧下接分析记录；手机端仍纵向排列；添加／编辑自选股与修改密码按需打开弹窗。个股、大盘、综合分析通过页签切换，保留离线保护与重试。发起分析区域不再重复展示近期执行记录，保留当前任务进度及错误反馈；历史报告集中在分析记录表。最近连接使用较小字号，综合分析提交前不再展示快照提示，其服务端快照行为不变。
...
报告名称作为查看报告的链接，使用链接颜色及悬停／键盘焦点下划线提示；操作栏不再重复提供「查看」，仅提供私有下载与删除，回收站提供下载、恢复与永久删除。删除经确认后移入回收站，可恢复，不自动清空。回收站另有「永久删除」，二次确认后删除报告正文与私有附件，无法恢复；独立分析任务记录及引擎历史保留。开始永久删除后禁止恢复和读取报告；中途失败保留重试入口，重复请求不会重新发布旧报告。只有当前已启用账户可以操作自己的记录；保存中或关联执行仍进行中的记录不能删除。回收站记录不出现在正常列表和计数中；尚未开始永久删除时，其本人报告深链与私有下载仍可用，当前任务的报告入口会提示报告在回收站。存储使用量不会因移入回收站而减少。

永久删除还需应用 `supabase/migrations/202609210002_cloud_report_purge.sql`，并部署 `/api/reports`；使用既有服务端 Supabase 环境变量，密钥不会进入浏览器。保留本人可见的发布墓碑以阻止重试复活，列表隐藏已永久删除项。永久删除无法通过回滚前端恢复。
