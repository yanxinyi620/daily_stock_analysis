...
操作提供查看、私有下载与删除。删除经确认后移入回收站，可恢复，不自动清空。回收站另有「永久删除」，二次确认后删除报告正文与私有附件，无法恢复；独立分析任务记录及引擎历史保留。开始永久删除后禁止恢复和读取报告；中途失败保留重试入口，重复请求不会重新发布旧报告。只有当前已启用账户可以操作自己的记录；保存中或关联执行仍进行中的记录不能删除。回收站记录不出现在正常列表和计数中；尚未开始永久删除时，其本人报告深链与私有下载仍可用，当前任务的报告入口会提示报告在回收站。存储使用量不会因移入回收站而减少。
