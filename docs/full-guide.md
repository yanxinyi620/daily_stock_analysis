...
- ✨ **综合分析** - 首页“综合分析”会冻结当前全部自选股、策略、报告语言、复盘市场和“推送通知”开关，依次完成个股分析、大盘复盘、综合报告保存和可选通知。任一股票或大盘复盘失败不会中断其余项目，最终报告会注明失败项；提交时若所选股票、另一综合任务或大盘复盘正在运行，则整单拒绝，不会只提交一部分。关闭“推送通知”时仍会生成并保存综合报告，但不会发送邮件或其他通知。
- 📄 **综合报告历史** - 从首页历史记录打开综合分析时，页面以和个股报告一致的卡片样式展示执行摘要、个股成功/失败数、大盘复盘状态、股票范围和通知开关，默认不展开完整正文，也不显示仅适用于单股的自选、追问、趋势和策略点位操作；点击“完整分析报告”阅读 Markdown，或按当前自选股与首页设置重新运行。
