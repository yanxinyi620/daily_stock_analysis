...
1. 选取已过冷却期（默认 14 天）的个股 `AnalysisHistory` 记录；大盘复盘和综合分析不属于单股方向验证对象，默认排除
...
回测在每日分析流程完成后自动触发（非阻塞，失败不影响通知推送）。也可通过 API 手动触发。Web 页面使用后台任务并轮询短状态请求，补取外部日线耗时较长时不会因通用 HTTP 请求时限提前误报失败；刷新页面会从当前服务进程恢复活动任务、继续等待并禁用重复提交。结果列表与汇总使用相同的个股过滤口径；旧汇总包含已排除的非个股记录时，会按当前有效个股结果重新计算。原同步 `/api/v1/backtest/run` 接口继续保留以兼容已有调用方。

Web/API 发起的回测会在独立 `backtest_runs` 表保存轻量运行历史，包括参数、状态、起止时间和处理汇总，不复制行情、逐股结果或完整日志。Web 回测页可在“回测结果 / 运行历史”之间切换，并可按历史参数重新运行。该能力从启用后开始记录，不回填此前任务。
...
手机访问同一服务的 `/m`（例如 `http://局域网地址:8000/m`）可进入移动日常使用端。它与桌面 WebUI 共享认证、API、任务、报告、自选和问股会话，不会生成第二份业务数据。

- 底部五个入口为「首页 / 自选 / 问股 / 任务 / 我的」。
- 首页可提交单股分析、全部自选的综合分析、大盘复盘和智能选股；推送选择沿用桌面首页状态。
- 自选页支持添加、移除、单股分析、全部分析和仅分析尚无报告的股票。
- 任务页按进行中、已完成、失败/取消分组；完成任务可进入移动报告。
- 报告首屏优先展示评分、操作、趋势、风险和摘要，完整 Markdown 仅在点击后加载。
- 「我的」只提供日常入口、主题和语言切换；模型渠道、密钥、数据源和通知目标仍在桌面设置页维护，手机端不显示敏感值。

浏览器支持时可将页面“添加到主屏幕”；PWA 只提供独立窗口和启动图标，不缓存 API、任务或报告。局域网访问仍应启用 Web 密码，跨公网使用时建议通过 HTTPS 反向代理或可信 VPN，不要直接暴露 8000 端口。
...
- 🔎 **自选股快速添加** - 自选页添加框支持按代码或名称匹配股票；点击候选只会把规范代码加入自选，不会立即运行分析。直接输入合法代码后按 Enter 或点击加号仍可添加。
...
- ✨ **综合分析** - 首页“综合分析”会冻结当前全部自选股、策略、报告语言、复盘市场和“推送通知”开关，依次完成个股分析、大盘复盘、综合报告保存和可选通知。成功运行会形成“每只成功个股一条历史 + 一条大盘复盘历史 + 一条综合历史”，其中大盘历史与综合历史可通过同一任务 ID 追溯；任一股票或大盘复盘失败不会中断其余项目，最终报告会注明失败项。提交时若所选股票、另一综合任务或大盘复盘正在运行，则整单拒绝，不会只提交一部分。关闭“推送通知”时仍会生成并保存这些报告，但不会发送邮件或其他通知。
- 📄 **综合报告历史** - 从首页历史记录打开综合分析时，页面以和个股报告一致的卡片样式展示执行摘要、个股成功/失败数、大盘复盘状态、股票范围和通知开关，默认不展开完整正文，也不显示仅适用于单股的自选、追问、价格趋势和策略点位操作。顶部统一提供“重新分析”“历史记录”“完整分析报告”：重新分析使用当前自选股和当前首页设置；历史记录打开综合任务专用时间线，按时间查看股票范围、成功/失败数、大盘状态、综合评分、通知状态和模型，并可切换到任一次完整报告。
