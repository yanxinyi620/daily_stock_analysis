...
- Market Light 通用快照支持 `cn` / `hk` / `us` / `jp` / `kr`；告警目标仍只支持 `cn` / `hk` / `us`。
- Web 告警市场下拉不展示 `jp` / `kr`；后端告警专用 `normalize_market_alert_region()` 对 `jp` / `kr` 返回显式 unsupported 错误，通用 `normalize_market_region()` 则接受这两个市场。
