...
- [改进] 分析记录保留报告名称链接并强化交互提示，移除操作栏重复的查看入口。
...
- [改进] 精简发起分析区域，缩小最近连接字号，移除近期执行记录与综合分析提交提示，保留当前任务进度和错误反馈。

- [改进] 云端首页改为左右工作台，自选股编辑与账户改密按需弹窗；报告状态合并为一列，时间移除副文案，个股报告直接显示股票名称。
- [新功能] 分析记录支持本人移入回收站与恢复，保留报告和私有附件；数据库校验账户、进行中状态与发布重试并发，禁止自动恢复已删除记录。

- [改进] 云端首页合并分析报告与最近发布为分析记录表，区分分析结果和保存状态，保留失败记录与私有下载，并支持分页和窄屏横向滚动。
...
- [新功能] 云端 Runner 接入综合分析，提交时原子固定用户自选股快照，复用原个股与大盘编排并原子保存部分完成摘要；新增私有历史字段扩宽迁移，保留原本地行为。
- [修复] 云端综合任务核对成功子项历史，失联后不启动后续股票；恢复发布包校验当前任务关联，避免误放文件被发布。

- [新功能] 云端 Runner 新增单市场大盘复盘，复用原分析入口与云端历史，支持五个市场选择并保留离线保护、幂等提交和模板降级提示。

- [新功能] 新增单一本地 Runner 单股云端执行闭环、Vercel 轻量任务 API、会话心跳与离线拒绝；分析历史保存至 Supabase 私有 schema，报告发布与执行成功原子提交，保留原 CLI/桌面功能。
- [测试] 增加 Runner 会话、重复提交、用户隔离、私有引擎存储与前端离线状态回归；修正受全局 HTTP mock 和固定日期影响的既有测试。

- [文档] 将后续云端改造收敛为本地单一 Runner 与交易日 Actions 综合分析，明确离线拒绝、发布状态兼容和阶段验收；弃用 Vercel Python Functions 分析计划，保留历史部署证据。

- [文档] 新增单用户 Vercel Python Functions 验证计划，以真实打包、单股/复盘/流式问股和跨部署留存作为验收关卡，暂不迁移全部功能。

- [文档] 完成正式子域名 stock.xinyilab.top 的 DNS、HTTPS 与认证地址配置，记录双用户浏览器验收和网络边界，保留原生产入口。

- [文档] 记录独立 Supabase 项目的加密备份恢复演练，补充默认授权叠加、托管权限和 Storage 策略恢复的验收要求。

- [测试] 新增默认离线的 Supabase 权限验收脚本，覆盖现有报告的双用户隔离、匿名访问、发布 RPC 拒绝及会话清理。

- [修复] 测试环境按用例恢复环境变量及数据库单例，并在 SQLite 打开前拒绝访问应用数据库（含自定义路径、URI 和软链接），防止回归测试回落到历史库。
- [测试] 修正首页系统报告置顶与告警仅支持 A 股/港股/美股的陈旧断言。
- [修复] 更新 Web 同主版本安全依赖，并拒绝本地登录返回地址中的反斜杠与控制字符；保留 React/Vite 和 Supabase 的 Node 20 兼容版本。

- [新功能] 新增可选 Supabase 报告发布与 React/Vite 云端阅读模式，支持预建用户登录、自选股、私有附件、发布重试及用户级 RLS；网页 Actions 投递保持关闭。
