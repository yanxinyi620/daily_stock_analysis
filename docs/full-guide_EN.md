...
> Generation fallback diagnostics: when regular analysis starts with a local CLI backend and falls back to LiteLLM, the run flow records the sanitized CLI failure, failed LiteLLM model attempts, and the final successful model in execution order. The Usage page persists only calls with token telemetry; an API fallback such as DeepSeek is attributed to the actual final model with `transport=litellm`, not to the originally selected local CLI. Missing attempt chains are not backfilled into older history.
...
- **Composite Analysis** - The Home action freezes the current watchlist, strategy, report language, market scope, and notification switch, then runs stock analysis, market review, report persistence, and one optional combined notification. Individual stock or market-review failures are listed without aborting the remaining work. If any selected stock, another composite run, or market review is already active, the whole submission is rejected atomically. With notifications off, the report is still generated and saved.
- **Composite report history** - Opening a composite analysis from Home history uses the same card language as stock reports to show the execution summary, stock success/failure counts, market-review state, stock scope, and notification setting. The full body stays collapsed by default, and stock-only watchlist, follow-up, trend, and strategy actions remain hidden. Use “Full analysis report” for the Markdown or rerun with the current Home settings.
- **Watchlist quick add** - Match stocks by code or name in the watchlist field. Selecting a suggestion adds its canonical code only and never starts analysis; a valid code can still be added with Enter or the plus button.
...
> Note: `POST /api/v1/backtest/run` adds `analysis_date_from` / `analysis_date_to` (`YYYY-MM-DD`) to filter candidates by analysis date range. When `analysis_date_from > analysis_date_to`, it returns 400 `invalid_params`. Backtest candidates, results, and summaries are stock-analysis records only; market-review and composite-analysis records are excluded because they do not represent a single-stock directional prediction. A legacy persisted summary whose count differs from the currently valid stock results is recomputed from those filtered results. The Web UI submits `POST /api/v1/backtest/tasks` and polls the task status so slow external daily-bar retrieval does not hit the shared request timeout. After a page refresh it queries the active task from the current server process, resumes polling, and keeps duplicate submission disabled; the synchronous `/run` endpoint remains available for compatibility.
> Note: Web/API backtests persist lightweight task-level history in `backtest_runs`, including parameters, lifecycle timestamps, status, and execution counts without duplicating market data, per-stock results, or full logs. The Web page can switch between results and run history and rerun saved parameters. Recording starts after this feature is enabled; older runs are not backfilled.
