...
export function useAnalysisRecords(api: CloudData, user: string, page: number, size: number, revision: number) {
  const [rows, setRows] = useState<AnalysisRecord[]>([]);
  const [error, setError] = useState(false);
  const [count, setCount] = useState(0);
  const key = JSON.stringify([user, page, size]);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      inFlight = true;
      try {
        const result = await api.records(user, page, size);
        if (stopped) return;
  ...
  }, [api, user, page, size, revision, key]);
